...
### 🎯 Master's Thesis: Robot Visual Tracking and Communication System
**Status:** In Progress

**Challenge:** End-to-end computer vision pipeline from image capture to motor control on resource-constrained robotics hardware must meet hard real-time deadlines.

**Approach:**
- Designed multi-microcontroller architecture: **Raspberry Pi (vision) → ESP32 (communication) → STM32 (motor control)** for modular, low-latency perception-to-action flow
- Profiled and optimized communication stack (**SPI, UART, ESP-NOW**) to achieve sub-50ms end-to-end latency (target: < 30 ms)
- Identified and resolved bottlenecks in image capture/processing pipeline through systematic tracing
- Built Python-based real-time visualization suite for performance monitoring and cross-platform debugging
